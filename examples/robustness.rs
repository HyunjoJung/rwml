...
//! clean [`rdoc::Error`] from a real panic (the crate's panic-free contract), and
//! counting the structure produced.
//!
//! ```text
//! cargo run -p rdoc --example robustness -- file1.doc file2.docx ...
//! ```
...
fn main() {
    ...
    for path in std::env::args().skip(1) {
        ...
        let result = catch_unwind(AssertUnwindSafe(|| match rdoc::Document::open(&bytes) {
            Ok(doc) => {
                let _ = doc.text();
