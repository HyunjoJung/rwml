...
fn render_document(
    ...
    fonts: Option<&[Vec<u8>]>,
    options: &Options,
) -> rwml::Result<rwml::RenderedPdf> {
    match fonts {
        // The existing release validator uses --fixed-fonts with system fallback.
        Some(fonts) if options.font_paths.is_empty() => doc.try_to_pdf_with_fonts_and_report(fonts),
...
fn main() -> ExitCode {
    ...
    let rendered = match render_document(&doc, fixed_fonts.as_deref(), &options) {
    ...
    if let Some(path) = options.report_json {
        if let Err(e) = std::fs::write(&path, rendered.report.to_json()) {
...
mod tests {
    ...
    #[cfg(feature = "docx")]
    #[test]
    fn bundled_mode_retains_legacy_fallback_but_explicit_fonts_are_isolated() {
        let model = rwml::DocBuilder::new()
            .paragraph("Uncovered \u{1f600}")
            .build();
        let doc = rwml::Document::open(&rwml::write_docx(&model)).unwrap();
        let fonts = [rwml_fonts::noto_sans_kr_subset_with_hanja().to_vec()];
        let bundled = parse(&["input.docx", "--fixed-fonts"]).unwrap();
        let explicit = parse(&["input.docx", "--font", "regular.ttf"]).unwrap();
        let ordinary = parse(&["input.docx"]).unwrap();

        let legacy = render_document(&doc, Some(&fonts), &bundled).unwrap();
        assert_eq!(legacy.pdf, doc.try_to_pdf_with_fonts(&fonts).unwrap());
        assert!(render_document(&doc, Some(&fonts), &explicit).is_err());
        let default = render_document(&doc, None, &ordinary).unwrap();
        assert_eq!(default.pdf, doc.try_to_pdf().unwrap());
    }
