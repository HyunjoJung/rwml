...
def validate_metric_contract(contract: object) -> None:
    ...
    if (
        not isinstance(contract["implementation"], str)
        or contract["implementation"] not in METRIC_IMPLEMENTATIONS
    ):
        raise ValueError("integer visual metric implementation is invalid")
    for key in METRIC_CONTRACT_KEYS - {"schema", "implementation"}:
        if type(contract[key]) is not int or contract[key] != expected[key]:
...
def validate_metrics(evidence: object) -> None:
    ...
    if evidence["pages"] > evidence["pixels"]:
        raise ValueError("integer visual page count exceeds pixel count")
    ppm_keys = {
        key
    ...
    changed = evidence["changed_pixels"]
    absolute = evidence["absolute_error_sum"]
    squared = evidence["squared_error_sum"]
    maximum = evidence["max_channel_delta"]
    if (
        changed > absolute
        or not maximum <= absolute <= changed * 3 * maximum
        or not max(absolute, maximum * maximum) <= squared <= absolute * maximum
    ):
        raise ValueError("integer visual raw error counts are inconsistent")
    expected_base = {
    ...
    if color_samples != evidence["foreground_candidate_matched_1px"]:
