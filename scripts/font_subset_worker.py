...
def strict_json(payload: bytes):
    ...
    def integer(value):
        if len(value.lstrip("-")) > MAX_JSON_INTEGER_DIGITS:
            raise SubsetError("json_integer_bound")
        return int(value)
    ...
    try:
        value = json.loads(
            ...
            parse_constant=constant, parse_int=integer, parse_float=finite_float,
        )
    except (UnicodeDecodeError, json.JSONDecodeError, RecursionError) as error:
        raise SubsetError("invalid_json") from error
    nodes = 0
    pending = [(value, 1)]
    while pending:
        ...
        if nodes > MAX_JSON_NODES or depth > MAX_JSON_DEPTH:
            raise SubsetError("json_complexity_bound")
