...
try:
    ...
    from render_oracle_contract import (
        CAMPAIGN_CAPTURE_SCHEMA, _assert_path_neutral, _load_json, load_corpus_manifest,
    )
except ModuleNotFoundError:
    from scripts import font_subset_attestation as attestation
    from scripts import libreoffice_container as runtime
    from scripts import libreoffice_table_capture as table_capture
    from scripts import native_cff_attestation as native
    from scripts import pdf_font_resources as resources
    from scripts import render_validate as render
    from scripts import shared_oracle_fonts as shared
...
def check_fonts(
    ...
    pypdf: Path,
    *,
    timeout: float = FONT_CHECK_SECONDS,
) -> dict:
    if (
        type(timeout) not in (int, float)
        or not 0 < timeout <= FONT_CHECK_SECONDS
        or not math.isfinite(timeout)
    ):
        raise ValueError("campaign PDF font timeout is outside its bound")
    ...
    if cff_fonts:
        ...
        cff = native.attest_pdf(
            ...
            timeout=remaining(native.BATCH_SECONDS),
        )
...
def build_renderer(output: Path, *, timeout: float = 900) -> dict:
    if (
        type(timeout) not in (int, float)
        or not 0 < timeout <= 900
        or not math.isfinite(timeout)
    ):
        raise ValueError("native build timeout is outside its bound")
    deadline = time.monotonic() + timeout
    ...
    with tempfile.TemporaryDirectory(dir=scratch) as temporary:
        ...
        try:
            log = runtime.run_bounded(
                ...
                timeout=remaining_seconds(deadline, 900),
    ...
    rustc = (
        runtime.run_bounded(
            ["rustup", "run", "1.92.0", "rustc", "--version"], env=env,
            timeout=remaining_seconds(deadline, 30),
        )
        .decode()
        .strip()
    )
    remaining_seconds(deadline, 900)
    return {
        **identity(payload),
        "rustc": rustc,
        "features": ["render"],
...
def pdf_record(
    ...
    *,
    ...
    checks = check_fonts(
        payload, lock.fonts, sources, fonttools, pypdf,
        timeout=remaining_seconds(deadline, FONT_CHECK_SECONDS),
    )
...
def verify_native_outputs(output: Path, corpus, renderer: dict, lock, deadline: float) -> None:
    ...
    with tempfile.TemporaryDirectory(dir=scratch) as temporary:
        ...
        require_equal(
            renderer,
            build_renderer(binary, timeout=remaining_seconds(deadline, 900)),
            "independently rebuilt renderer",
        )
        for document in corpus.documents:
            retained = output / "cases" / document.case_id
            payload = source_payload(document)
            require_equal(
                payload,
                runtime.read_regular_file(retained / STAGED_INPUT_NAME, document.input_bytes),
                "replay input",
            )
            with tempfile.TemporaryDirectory(dir=root) as case_temporary:
                directory = Path(case_temporary)
                write_new(directory / STAGED_INPUT_NAME, payload)
                runtime.run_bounded(
                    native_command(
                        binary, directory / STAGED_INPUT_NAME,
                        directory / "native.pdf", directory / "native-report.json",
                        [output / "fonts" / entry["name"] for entry in lock.fonts],
                    ),
                    timeout=remaining_seconds(deadline, NATIVE_WALL_SECONDS),
                    cwd=ROOT,
                )
                remaining_seconds(deadline, MAX_CAMPAIGN_SECONDS)
                for name, maximum in (
                    ("native.pdf", resources.worker.MAX_PDF_BYTES),
                    ("native-report.json", 1024 * 1024),
                ):
                    require_equal(
                        runtime.read_regular_file(directory / name, maximum),
                        runtime.read_regular_file(retained / name, maximum),
                        f"native {name} replay",
                    )
        require_equal(
            {key: renderer[key] for key in ("bytes", "sha256")},
            identity(runtime.read_regular_file(binary, MAX_RENDERER_BYTES)),
            "replay executable after verification",
        )
    ...
    *,
    ...
    if verify:
        ...
        renderer = build_renderer(
            output / "renderer", timeout=remaining_seconds(deadline, 900),
        )
        (output / "fonts").mkdir()
    ...
    if verify:
        verify_native_outputs(output, corpus, renderer, lock, deadline)
    require_equal(
