...
def validate_diagnostic_contract(value: object) -> None:
    ...
    while pending:
        ...
        if type(actual) is not type(expected):
            raise ValueError("PDF diagnostic contract value type is invalid")
        if isinstance(expected, dict):
            pending.extend((actual[key], item) for key, item in expected.items())
...
def _bounded_sequence(value: object, length: int, name: str) -> tuple[object, ...]:
    ...
    try:
        result = tuple(islice(value, length + 1))
...
def validate_page_geometry_metrics(value: object) -> None:
    ...
    for axis in POINT_AXES:
        ...
        if type(deltas[axis]) is not int or deltas[axis] != expected:
    ...
    if (
        type(value["rotation_delta_degrees"]) is not int
        or value["rotation_delta_degrees"] != expected_rotation
    ):
...
def _validate_tokens(tokens: Sequence[str], name: str) -> tuple[str, ...]:
    ...
    try:
        result = tuple(islice(tokens, MAX_SEMANTIC_TOKENS + 1))
    except TypeError as error:
        raise ValueError(f"semantic {name} tokens are invalid") from error
...
def _validate_ratio_fields(value: dict[str, int], prefix: str) -> None:
    ...
    if value["semantic_exact"] and not reference_items == candidate_items == matched_items:
        raise ValueError(f"semantic exact flag contradicts {prefix} counts")
...
def validate_semantic_report(value: object) -> None:
    ...
    if value["semantic_comparable_pages"] + value["semantic_one_sided_empty_pages"] != pages:
        raise ValueError("semantic comparable and empty pages do not cover the report")
    if value["semantic_exact_pages"] > value["semantic_comparable_pages"]:
        raise ValueError("semantic exact pages exceed comparable pages")
...
def _text_geometry_bucket_bounds(bucket: int) -> tuple[int, int]:
    magnitude = abs(bucket)
    if magnitude <= TEXT_GEOMETRY_EXACT_LIMIT_MILLIPOINTS:
    ...
    if magnitude <= TEXT_GEOMETRY_MIDDLE_LIMIT_MILLIPOINTS:
        ...
        low = (
            ...
            if magnitude == width else magnitude - width // 2
        )
        high = min(TEXT_GEOMETRY_MIDDLE_LIMIT_MILLIPOINTS, magnitude + width // 2 - 1)
    elif magnitude <= TEXT_GEOMETRY_OUTER_LIMIT_MILLIPOINTS:
        width = TEXT_GEOMETRY_OUTER_BUCKET_MILLIPOINTS
        low = max(TEXT_GEOMETRY_MIDDLE_LIMIT_MILLIPOINTS + 1, magnitude - width // 2)
        high = min(TEXT_GEOMETRY_OUTER_LIMIT_MILLIPOINTS, magnitude + width // 2 - 1)
    else:
        low = TEXT_GEOMETRY_OUTER_LIMIT_MILLIPOINTS + 1
        high = MAX_TEXT_GEOMETRY_DELTA_MILLIPOINTS
    return (-high, -low) if bucket < 0 else (low, high)
...
def validate_unique_text_geometry_metrics(value: object) -> None:
    ...
    for axis in TEXT_GEOMETRY_AXES:
        ...
        buckets = {row["delta_millipoints"]: row["count"] for row in rows}
        if (
            summary["negative_overflow_items"]
            != buckets.get(-TEXT_GEOMETRY_OVERFLOW_MILLIPOINTS, 0)
            or summary["positive_overflow_items"]
            != buckets.get(TEXT_GEOMETRY_OVERFLOW_MILLIPOINTS, 0)
        ):
            raise ValueError(f"text geometry overflow counts contradict histogram: {axis}")
        if matched:
            minimum, maximum = summary["min"], summary["max"]
            if not (
                (matched - 1) * minimum + maximum
                <= summary["sum"]
                <= (matched - 1) * maximum + minimum
            ):
                raise ValueError(f"text geometry sum contradicts extrema: {axis}")
            if (
                _text_geometry_bucket(minimum) not in buckets
                or _text_geometry_bucket(maximum) not in buckets
            ):
                raise ValueError(f"text geometry extrema lack histogram coverage: {axis}")
            sum_low = sum_high = 0
            for bucket, bucket_count in buckets.items():
                low, high = _text_geometry_bucket_bounds(bucket)
                low, high = max(low, minimum), min(high, maximum)
                if low > high:
                    raise ValueError(f"text geometry histogram contradicts extrema: {axis}")
                sum_low += low * bucket_count
                sum_high += high * bucket_count
            if not sum_low <= summary["sum"] <= sum_high:
                raise ValueError(f"text geometry sum contradicts histogram: {axis}")
