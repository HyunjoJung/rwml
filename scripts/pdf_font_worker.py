...
try:
    ...
    from scripts import font_subset_worker as common
...
def extract_resources(payload: bytes) -> tuple[list, list]:
    ...
    def is_name(raw, *names):
        # pypdf names and text strings can compare equal despite different PDF types.
        value = resolve(raw)
        return isinstance(value, NameObject) and value in names

    def add_font(raw):
        ref = identity(raw)
        value = dictionary(raw)
        require(
            value.get("/Type") is None or is_name(value.get("/Type"), "/Font"),
            "pdf_font_type",
        )
        require(
            is_name(value.get("/Subtype"), "/Type0", "/Type1", "/TrueType"),
            "pdf_font_kind",
        )
        fonts[ref] = value
    ...
    require(is_name(dictionary(root).get("/Type"), "/Catalog"), "pdf_catalog")
    ...
    while stack:
        ...
        if isinstance(value, DictionaryObject):
            if resolve(value.get("/Type")) == "/Font":
                require(is_name(value.get("/Type"), "/Font"), "pdf_font_type")
    ...
    def blob(raw, kind):
        ...
        require(
            ...
            or is_name(filter_value, "/FlateDecode")
            or isinstance(filter_value, ArrayObject)
            and len(filter_value) == 1
            and is_name(filter_value[0], "/FlateDecode"),
        ...
        if kind == "cid-cff":
            require(
                is_name(value.get("/Subtype"), "/CIDFontType0C"), "pdf_program_kind"
            )
    ...
    for ref, font in sorted(fonts.items()):
        ...
        if subtype == "Type0":
            require(
                is_name(encoding, "/Identity-H", "/Identity-V"), "pdf_encoding_unsupported"
            )
            ...
            require(is_name(target.get("/Type"), "/Font"), "pdf_font_type")
        ...
        require(
            descriptor.get("/Type") is None
            or is_name(descriptor.get("/Type"), "/FontDescriptor"),
            "pdf_descriptor_type",
        )
        keys = [
            key
