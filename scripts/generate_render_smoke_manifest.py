...
def _canonical_json(value: object) -> bytes:
    return batch.canonical_json(value)
...
def _validate_output_location(path: Path) -> None:
    if path.parent.resolve() != SOURCE_MANIFEST.parent.resolve():
        raise ValueError("smoke manifest must remain in the source corpus directory")
    if path.resolve() == SOURCE_MANIFEST.resolve():
        raise ValueError("smoke output must not replace the source manifest")


def refresh(path: Path = OUTPUT_MANIFEST) -> None:
    _validate_output_location(path)
    batch.atomic_write(path, expected_manifest_bytes())
...
def check(path: Path = OUTPUT_MANIFEST) -> bool:
    try:
        ...
        actual = _read_bounded_regular_file(path, len(expected))
        if actual != expected:
