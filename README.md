...
switch tails, and malformed hyperlink syntax reports `UnsupportedSwitch`; cached
...
numeric ordinary document bookmark operands, literal `DEFINED(expr)` checks for parser-local literal expressions,
source-order prior `SET`/`ASK` bookmark names, and ordinary document bookmark names, guarded scalar/table numeric `IF` branch selection skips unsupported or span-unsafe unselected branches, `+`, `-`, `*`, `/`, `^`, parentheses, unary signs,
...
unquoted `TA`/`XE` marker operands, with field-result format switches render as
...
numbering/list syntax reports `UnsupportedSwitch`, while richer `AUTONUMOUT`/`BIDIOUTLINE`
outline semantics and richer `LISTNUM`
