...
literal `DEFINED(expr)` checks for parser-local literal expressions and source-order prior bookmark names, guarded scalar/table numeric `IF` branch selection skips unsupported unselected branches, `+`, `-`, `*`, `/`, `^`, parentheses, unary signs,
