...
impl StyleSheet {
    ...
    pub(crate) fn parse(table: &[u8], fc_stshf: usize, lcb_stshf: usize) -> StyleSheet {
        ...
        for istd in 0..n {
            heading[istd] = resolve_level(&descs, istd, &mut vec![false; n]);
...
/// then by name. `visited` guards against base-style cycles.
fn resolve_level(
    descs: &[Option<StyleDescription>],
    istd: usize,
    visited: &mut [bool],
) -> Option<u8> {
    if istd >= descs.len() || visited.get(istd).copied().unwrap_or(true) {
        return None;
    }
    visited[istd] = true;
    ...
    if d.sgc == 1 && d.istd_base != 0x0FFF && d.istd_base as usize != istd {
        if let Some(n) = resolve_level(descs, d.istd_base as usize, visited) {
            return Some(n);
        }
    }
...
mod tests {
    ...
    fn resolve_by_sti_and_base_chain() {
        ...
        let n = descs.len();
        let lvl = |i| resolve_level(&descs, i, &mut vec![false; n]);
    ...
    fn cycle_guard() {
        ...
        assert_eq!(resolve_level(&descs, 0, &mut [false; 2]), None);
    }
}
