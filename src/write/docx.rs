...
pub(crate) fn to_docx(model: &crate::DocModel) -> Vec<u8> {
    ...
    if ctx.has_list {
        ...
        // Relate numbering to the document (read-by-path doesn't need it, but
        // strict consumers like Word expect the relationship to exist).
        doc_rels.push(Rel {
    ...
    if !doc_rels.is_empty() {
        pkg.add_rels("word/_rels/document.xml.rels", doc_rels);
    }
    pkg.add_rels(
        "_rels/.rels",
        vec![Rel {
...
mod tests {
    ...
    fn round_trips_structure_through_docx() {
        ...
        let link = Paragraph {
            ...
            runs: vec![Run {
                ...
                field: FieldRole::Hyperlink {
                    url: "https://www.g2b.go.kr/".to_string(),
                },
                ..Run::default()
            }],
        };
        ...
        let png = vec![0x89, 0x50, 0x4E, 0x47, 0x0D, 0x0A, 0x1A, 0x0A, 1, 2, 3, 4];
        ...
        assert!(matches!(
            l.runs.iter().find(|r| r.text == "조달청").map(|r| &r.field),
            Some(FieldRole::Hyperlink { url }) if url == "https://www.g2b.go.kr/"
        ));
