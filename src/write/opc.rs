...
impl Package {
    ...
    /// Serialize the whole package to ZIP bytes. Infallible for the in-memory
    /// buffer; yields an empty `Vec` on the unreachable write error rather than
    /// panicking (honoring the crate's panic-free contract).
    pub(crate) fn into_zip(self) -> Vec<u8> {
        self.try_into_zip().unwrap_or_default()
    }

    fn try_into_zip(self) -> std::io::Result<Vec<u8>> {
