...
class IntegerVisualMetricTests(unittest.TestCase):
    def test_contract_rejects_boolean_float_and_untyped_implementation(self):
        for key, value in (
            ("match_radius_pixels", True),
            ("blur_kernel_pixels", 3.0),
            ("implementation", []),
        ):
            with self.subTest(key=key, value=value):
                contract = metrics.metric_contract()
                contract[key] = value
                with self.assertRaises(ValueError):
                    metrics.validate_metric_contract(contract)
    ...
    def test_optional_numpy_matches_reference_or_reports_unavailability(self):
        if metrics.numpy_module() is None:
            reference = b"\xff\xff\xff"
            candidate = b"\x00\x00\x00"
            self.assertEqual(
                metrics.image_metrics(reference, candidate, 1, 1),
                metrics.image_metrics_python(reference, candidate, 1, 1),
            )
            with self.assertRaisesRegex(ValueError, "NumPy is unavailable"):
                metrics.image_metrics_numpy(reference, candidate, 1, 1)
            return
    ...
    def test_validator_rejects_impossible_raw_counts_even_with_recomputed_ratios(self):
        blank = metrics.image_metrics_python(b"\xff\xff\xff", b"\xff\xff\xff", 1, 1)
        black = metrics.image_metrics_python(b"\x00\x00\x00", b"\x00\x00\x00", 1, 1)
        cases = (
            (blank, {"pages": 2}),
            (blank, {"changed_pixels": 1, "mismatch_ppm": metrics.PPM}),
            (blank, {"max_channel_delta": 255}),
            (black, {"foreground_matched_color_samples": 0}),
        )
        for original, updates in cases:
            with self.subTest(updates=updates):
                with self.assertRaises(ValueError):
                    metrics.validate_metrics({**original, **updates})
