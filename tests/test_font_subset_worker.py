...
class FontSubsetWorkerTests(unittest.TestCase):
    def test_json_requires_utf8_finite_numbers_and_bounded_integers(self):
        for payload in (
        ...
        self.assertEqual(worker.strict_json(b"9" * 128), int("9" * 128))
        self.assertEqual(worker.strict_json(b'{"value":0.125}'), {"value": 0.125})

    def test_json_depth_and_node_limits_are_explicit(self):
        worker.strict_json(b"[" * 63 + b"0" + b"]" * 63)
        for depth in (64, 5000):
            with self.subTest(depth=depth), self.assertRaises(worker.SubsetError):
                worker.strict_json(b"[" * depth + b"0" + b"]" * depth)
        with mock.patch.object(worker, "MAX_JSON_NODES", 4, create=True):
            self.assertEqual(worker.strict_json(b"[0,1,2]"), [0, 1, 2])
            with self.assertRaises(worker.SubsetError):
                worker.strict_json(b"[0,1,2,3]")
