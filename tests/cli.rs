...
#[cfg(feature = "render")]
fn run_rdoc_render(args: &[&str]) -> std::process::Output {
    let out = run_rdoc(args);
    ...
    Command::new(std::env::var("CARGO").unwrap_or_else(|_| "cargo".to_string()))
        ...
        .args(args)
        .output()
        .expect("run render-enabled rdoc cli")
}
...
fn cli_extract_prints_plain_text() {
    ...
    let out = run_rdoc(&["extract", path.to_str().unwrap()]);
...
fn cli_diagnose_prints_report_json() {
    ...
    let out = run_rdoc(&["diagnose", path.to_str().unwrap()]);
...
fn cli_convert_supports_txt_markdown_and_html() {
    ...
    let txt = run_rdoc(&["convert", path, "txt"]);
    assert!(
        txt.status.success(),
        "stderr={}",
    ...
    let md = run_rdoc(&["convert", path, "md"]);
    assert!(
        md.status.success(),
        "stderr={}",
    ...
    let html = run_rdoc(&["convert", path, "html"]);
    assert!(
        html.status.success(),
        "stderr={}",
...
fn cli_to_docx_writes_reopenable_docx() {
    ...
    let out = run_rdoc(&["to-docx", input.to_str().unwrap(), output.to_str().unwrap()]);
    ...
    let doc = rdoc::Document::open(&converted).expect("converted docx reopens");
