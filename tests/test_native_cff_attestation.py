...
class NativeCFFTests(unittest.TestCase):
    def test_namespace_imports_work_without_a_scripts_path_override(self):
        ...
        for module in ("cff_mapping_worker", "native_cff_attestation"):
            with self.subTest(module=module):
                result = subprocess.run(
                    [
                        ...
                        "native.resources.tool_lock(); native.attestation.tool_lock()",
                        str(root),
                    ],
    ...
    def test_invalid_timeouts_fail_before_input_reads_or_worker_execution(self):
        for timeout in (True, False, None, "1", 1j, 0, -1, math.inf, math.nan, 31, 2**4096):
            with self.subTest(timeout=repr(timeout)[:32]):
                with mock.patch.object(
                    native.runtime, "read_regular_file", side_effect=AssertionError("input read")
                ) as read:
                    with self.assertRaises(ValueError):
                        native.discover_program(
                            b"program", b"cmap", b"source", {},
                            Path("fonttools"), Path("pypdf"), timeout=timeout,
                        )
