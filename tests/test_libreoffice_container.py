...
class LibreOfficeContainerTests(unittest.TestCase):
    ...
    def test_process_limits_reject_nonfinite_and_noncanonical_values_before_spawn(self):
        limits = [
            {"timeout": value}
            for value in (float("inf"), float("nan"), True, "5", None, 0, -1)
        ] + [
            {"stdout_limit": value}
            for value in (True, 1.5, "100", None, 0, oracle.MAX_CAPTURE_BYTES + 1)
        ]
        for options in limits:
            with self.subTest(options=options), mock.patch.object(
                oracle.subprocess, "Popen", side_effect=AssertionError("unexpected spawn")
            ) as spawn:
                with self.assertRaisesRegex(ValueError, "limits are invalid"):
                    oracle.run_bounded([sys.executable, "-c", "pass"], **options)
                spawn.assert_not_called()

    def test_regular_file_reader_preserves_binary_bytes_and_enforces_size(self):
        payload = b"\x00\r\n\x1a\xff\n"
        with tempfile.TemporaryDirectory() as temporary:
            path = pathlib.Path(temporary) / "payload"
            path.write_bytes(payload)
            self.assertEqual(oracle.read_regular_file(path, len(payload)), payload)
            with self.assertRaises(ValueError):
                oracle.read_regular_file(path, len(payload) - 1)
            path.write_bytes(b"")
            with self.assertRaises(ValueError):
                oracle.read_regular_file(path, 100)
