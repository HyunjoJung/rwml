...
class RenderSmokeManifestTests(unittest.TestCase):
    ...
    def test_check_rejects_stale_and_symlinked_output(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with mock.patch.object(generator, "SOURCE_MANIFEST", root / SOURCE.name):
                generator.refresh(output)
                with mock.patch.object(
                    generator, "load_corpus_manifest", wraps=generator.load_corpus_manifest
                ) as load:
                    self.assertTrue(generator.check(output))
                    load.assert_any_call(output)
    ...
    def test_custom_location_must_preserve_source_relative_paths(self):
        with tempfile.TemporaryDirectory() as tmp:
            output = pathlib.Path(tmp) / "smoke.json"
            with self.assertRaisesRegex(ValueError, "source corpus"):
                generator.refresh(output)
    ...
    def test_refresh_cannot_replace_the_parent_manifest(self):
        with tempfile.TemporaryDirectory() as temporary:
            root = pathlib.Path(temporary) / "corpus"
            shutil.copytree(SOURCE.parent, root)
            source = root / SOURCE.name
            original = source.read_bytes()
            with mock.patch.object(generator, "SOURCE_MANIFEST", source):
                with self.assertRaisesRegex(ValueError, "source manifest"):
                    generator.refresh(source)
            self.assertEqual(source.read_bytes(), original)

    def test_both_cli_import_modes_check_the_same_manifest(self):
        for command in ((str(SCRIPT),), ("-m", "scripts.generate_render_smoke_manifest")):
            with self.subTest(command=command):
                result = subprocess.run(
                    [sys.executable, "-B", *command, "--check"],
