...
class SharedOracleFontTests(unittest.TestCase):
    def test_module_imports_without_inserting_the_scripts_directory(self):
        result = subprocess.run(
            [
                sys.executable, "-I", "-B", "-c",
                "import sys; sys.path.insert(0, sys.argv[1]); "
                "from scripts import shared_oracle_fonts as fonts; "
                "from scripts.render_oracle_contract import _read_bounded_regular_file; "
                "assert fonts._read_regular_file is _read_bounded_regular_file; "
                "assert len(fonts.load_lock().fonts) == 10",
                str(ROOT),
            ],
