...
class PDFFontIntegrationTests(unittest.TestCase):
    ...
    def test_pdf_names_cannot_be_replaced_by_equal_text_strings(self):
        from pypdf.generic import TextStringObject

        cases = (
            ("catalog", "Type1", "/Type", "/Catalog", "pdf_catalog"),
            ("font", "Type1", "/Type", "/Font", "pdf_font_type"),
            ("descriptor", "Type1", "/Type", "/FontDescriptor", "pdf_descriptor_type"),
            ("descendant", "CIDFontType0", "/Type", "/Font", "pdf_font_type"),
            ("program", "CIDFontType0", "/Subtype", "/CIDFontType0C", "pdf_program_kind"),
            ("program", "Type1", "/Filter", "/FlateDecode", "pdf_filter_unsupported"),
        )
        for target, kind, key, value, reason in cases:
            def mutate(s):
                objects = {
                    "catalog": s["writer"].root_object,
                    "font": s["font"],
                    "descriptor": s["descriptor"],
                    "program": s["program_ref"].get_object(),
                }
                if target == "descendant":
                    obj = s["font"]["/DescendantFonts"][0].get_object()
                else:
                    obj = objects[target]
                obj[s["name"](key)] = TextStringObject(value)

            with self.subTest(target=target, key=key):
                self.rejected(mutate, reason, kind=kind)

    def test_filter_array_requires_name_objects(self):
        from pypdf.generic import TextStringObject

        self.rejected(
            lambda s: s["program_ref"].get_object().update(
                {s["name"]("/Filter"): s["array"]([TextStringObject("/FlateDecode")])}
            ),
