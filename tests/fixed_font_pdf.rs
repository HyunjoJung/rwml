...
#[test]
fn fixed_pdf_ignores_undecodable_blobs_without_changing_valid_font_order() {
    let model = DocBuilder::new().paragraph("Visible text").build();
    let valid = fonts();
    let expected = rwml::try_render_pdf_with_fixed_fonts_and_report(&model, &valid).unwrap();
    let mut mixed = vec![vec![], vec![1, 2, 3, 4]];
    mixed.extend(valid);
    let actual = rwml::try_render_pdf_with_fixed_fonts_and_report(&model, &mixed).unwrap();
    assert_eq!(actual.pdf, expected.pdf);
    assert_eq!(actual.report.to_json(), expected.report.to_json());
}
