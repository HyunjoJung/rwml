...
class FontSubsetAttestationTests(unittest.TestCase):
    def test_namespace_import_works_without_a_scripts_path_override(self):
    ...
    def test_invalid_timeouts_fail_before_reading_tools_or_launching_workers(self):
        program, source = b"%!FontType1-test", b"OTTOsource"
        entry = {
            "bytes": len(source), "sha256": worker.digest(source),
            "postscript_name": "Locked-CJK", "sfnt_revision": 65536,
        }
        for timeout in (True, False, None, "1", 1j, 0, -1, math.inf, math.nan, 31, 2**4096):
            with self.subTest(timeout=repr(timeout)[:32]):
                with mock.patch.object(
                    attestation, "wheel_payload", side_effect=AssertionError("tool read")
                ) as tool:
                    with self.assertRaises(ValueError):
                        attestation.attest_program(
                            program, source, entry, Path("unused.whl"), timeout=timeout
                        )
