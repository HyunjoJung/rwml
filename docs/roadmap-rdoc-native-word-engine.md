...
- `Document::fields()` extracts simple and common complex `.docx` body, note,
  ...
  literal `DEFINED(expr)` checks for parser-local literal expressions and source-order prior bookmark names, guarded scalar/table numeric `IF` branch selection skips unsupported unselected branches, `+`, `-`, `*`, `/`, `^`, parentheses, unary signs, literal comparison operators (`=`, `<>`, `<`, `<=`, `>`, `>=`),
...
- `Document::report()` includes field-kind counts, and unsupported-field
  ...
  literal `DEFINED(expr)` checks for parser-local literal expressions and source-order prior bookmark names, guarded scalar/table numeric `IF` branch selection skips unsupported unselected branches, `+`, `-`,
...
- Generated running footer page numbers (`DocSetup::page_numbers`) and body
  ...
  guarded scalar/table numeric `IF` branch selection skips unsupported unselected branches,
