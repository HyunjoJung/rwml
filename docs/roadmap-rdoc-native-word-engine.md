...
- `Document::fields()` extracts simple and common complex `.docx` body fields
  ...
  syntax preserving cached text with `NoComputedResult` diagnostics, `TOC \c` full-caption entries
  ...
  `numberInDash`, `decimalFullWidth`, lower/upper letter, lower/upper roman,
  ordinal/cardinal text) on those section starts and single-section final
  `body/sectPr` page-number defaults,
...
- `Document::report()` includes field-kind counts, and unsupported-field
  ...
  computed values are not evaluated; cached `MERGEFIELD` results,
  ...
  applying layout offsets, validated `EQ \d` displacement controls render as
  ...
  insertions including Symbol `0xB7` bullet, while remaining display/layout cases report
...
- `Document::fill_template_fields()` is the higher-level template/mail-merge
  ...
  instructions.
- `Document::accept_all_revisions()` accepts tracked body revisions in
  `word/document.xml` by unwrapping inserted/move-to content, removing
  deleted/move-from content, and dropping property-change history while
  preserving current properties.
- `Document::reject_all_revisions()` rejects tracked body revisions in
  `word/document.xml` by removing inserted/move-to content, unwrapping
  deleted/move-from content, normalizing kept `w:delText` to `w:t`, and dropping
...
- `Document::add_image_jpeg()` and `Document::replace_image_jpeg()` extend the
...
- `DocBuilder` provides thin model-first authoring helpers for metadata, page
  ...
  size/orientation/margins/columns/document grids/text direction/title pages, page-number restarts/formats, explicit page breaks, section breaks that snapshot
...
- Remaining chart families beyond the current authored set and exact chart styling beyond
  3-D bar/column shapes and surface-family wireframes remain future
...
- Generated running footer page numbers (`DocSetup::page_numbers`) and body
  ...
  `MERGEFIELD`, metadata-backed document-info fields such as
