...
`ImageBuilder`, bar/stacked bar/100% stacked bar/3-D bar/column/stacked column/100% stacked column/3-D column/line/3-D line/area/stacked area/100% stacked area/3-D area/radar/radar-with-markers/filled radar/scatter/bubble/3-D bubble/pie/3-D pie/doughnut/surface/3-D surface/stock/pie-of-pie/bar-of-pie
...
- compute only low-risk fields at first (`PAGE` in renderer context and trusted
  ...
  `HYPERLINK` as link annotations, `FILENAME`/`MERGEFIELD`,
...
- compute bare default `.docx` `TOC` heading ranges, standalone bookmark-scoped
  ...
  text with `NoComputedResult` diagnostics, `TOC \c` full-caption entries and `TOC \a`
...
14. add pie-of-pie and bar-of-pie chart XML/render support on the shared
    chart data path;
15. add 3-D bar, 3-D column, 3-D line, and 3-D area chart XML/render support on the shared chart data path;
...
the model renderer draws bar, stacked bar, 100% stacked bar, 3-D bar, column, stacked column, 100% stacked column, 3-D column, line, 3-D line, area,
stacked area, 100% stacked area, 3-D area, radar, radar-with-markers, filled radar, scatter, bubble, 3-D bubble, pie, 3-D pie, doughnut, surface, 3-D surface,
...
- treat field-code `HYPERLINK` runs, cached body `FILENAME`/`MERGEFIELD`,
